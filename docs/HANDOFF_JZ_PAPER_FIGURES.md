...
| purpose | interpreter |
|---|---|
| NPE / jaxili / general plotting | `/lustre/fswork/projects/rech/nzu/ulx34io/envs/jaxili/bin/python` |
...
| **PS bias vs ℓmax** | ⬜ pending — `ps_submean_l37/tables` intact; PDF corrupt, PNG survives as reference |
| **BNT bin-1 tension vs cut** | ⬜ pending — `build_bnt_bin1_allareas_plot.py` + both tables verified intact; takes `--out-png/--out-pdf` so write to a new path |
| **3-statistic baryon-safe contours** | ⬜ **needs a new script.** `contours_PS_peaks_L1_baryons_{biased,BNT}.pdf` corrupt; the surviving `_unbiased.pdf` is Sept 2025 = *pre-correction*. `plot_getdist_triangles.py` is PS-only. Posterior samples exist |
...
## 6. Still outstanding beyond figures
