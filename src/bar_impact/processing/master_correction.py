...
def compute_coupling_matrix(
    ...
    if lmax > lmax_nyquist:
        print(f"Warning: Requested lmax={lmax} exceeds Nyquist limit "
    ...
    if use_cache:
        ...
        bin_hash = (hashlib.sha256(bin_edges.tobytes()).hexdigest()[:8]
                   if bin_edges is not None else 'unbinned')
...
def compute_power_spectra_master(
    ...
    **IMPORTANT:** Do NOT pre-multiply maps by the mask! NaMaster applies
    ...
    if verbose:
        print(f"Computing power spectra for {len(maps_dict)} bins with MASTER correction...")
