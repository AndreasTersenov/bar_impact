...
class DataVector:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "DataVector":
        ...
        return cls(
            ...
            cosmology_params=np.array(d["cosmology_params"])
            if "cosmology_params" in d
            else None,
...
class DataVectorCollection:
    ...
    def __post_init__(self):
        """Validate inputs."""
        self.data_vectors = np.atleast_2d(
            np.asarray(self.data_vectors, dtype=np.float64)
        )
