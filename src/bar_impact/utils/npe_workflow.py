...
STANDARD_COSMO_PARAMS = {
    ...
    'fiducial_values': jnp.array([[2.600e-01, 8.400e-01, -1.000e+00, 6.736e+01, 9.649e-01, 4.930e-02]])
}
...
def create_triangle_plot(samples, sample_label, output_path, 
                         param_config=None, color='blue', dpi=300):
    """
    ...
    if 'fiducial_values' in param_config:
        ...
        plot_kwargs['markers'] = {
            label: val for label, val in zip(param_config['labels'], true_params[0])
        }
...
def sample_and_save_posterior(posterior, observation, output_config):
    ...
    samples_np = np.array(samples)
    print(f"Generated {output_config['num_samples']} samples, shape: {samples_np.shape}")
...
def print_analysis_summary(config):
    ...
    print(f"Noisy: {config.get('noisy', False)}" + 
          (f" (noise level: {config.get('noise_level')})" if config.get('noisy') else ""))
    ...
    if 'new_normalization' in config and config['new_normalization']:
        print(f"New normalization: True")
