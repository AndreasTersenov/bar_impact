# PAPER: PS biased posteriors, all survey areas overlaid, full resolution

PAPER FIGURE. Power-spectrum posteriors for the baryonified observation, six masked footprints plus full sky, overlaid at full map resolution ell=37-1020 -- i.e. BEFORE any scale cut, so the baryon bias is present and grows with area. Sequential light-to-dark ramp encodes survey area, which is ordered. CAVEAT: the full-sky contour uses the healpy per-ell pipeline with no submean and no MASTER decoupling (submean subtracts a FOOTPRINT mean, which exists only under a mask), so it is not magnitude-comparable to the masked contours.

- **source**: `outputs/plots/contours_vs_area/contours_vs_area_ps_biased_l37-1020_with_fullsky`
- **generator commit**: `3d0116e`
- **generated**: 2026-07-30T11:59:55+00:00
- **published**: 2026-07-30T12:41:17+00:00 at repo `3d0116e`
