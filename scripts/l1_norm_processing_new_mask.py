#!/usr/bin/env python3
# filepath: /home/tersenov/software/bar_impact/scripts/l1_norm_processing_new.py
"""
...
def process_file(file_path, bin_number=2, noise_level=0.26, add_noise=True, 
    ...
    try:
        ...
        if apply_mask:
            ...
            # Note: We pass the mask to get_wtl1_sphere so it only computes
            # L1 norms on valid (unmasked) pixels. We do NOT multiply kg*mask
            # because that would create edge artifacts in the wavelet transform.
        else:
            mask = None
        
        _, l1norms = get_wtl1_sphere(
            kg, nscales=5, nbins=40, Mask=mask, min_snr=min_snr, max_snr=max_snr, 
...
def main():
    ...
    # Default values: bin1:[10,50], bin2:[40,100], bin3:[100,200], bin4:[150,300]
