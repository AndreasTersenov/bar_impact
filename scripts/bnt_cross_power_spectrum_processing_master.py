#!/usr/bin/env python3
"""
BNT Cross Power Spectrum Processing Script with MASTER Mode-Coupling Correction

This script processes cosmological data files to compute cross power spectra between 
different BNT-transformed redshift bins, with proper handling of masked sky observations:

1. Loads kappa maps from all 4 redshift bins
2. Adds shape noise (before BNT transform for physical consistency)
3. Applies BNT (Bernardeau-Nishimichi-Taruya) transform
4. Applies smooth apodization to mask edges (reduces mode-coupling artifacts)
5. Computes mode-coupling matrix (MCM) using NaMaster
6. Deconvolves pseudo-Cls to recover unbiased power spectra
...
- Bernardeau, Nishimichi & Taruya (2014): BNT transform
"""
...
def create_apodized_mask(nside=512, target_area_sqdeg=14000.0, center_coords=(0.0, 90.0),
    ...
    # Compute dot product between center vector and each pixel vector
...
def get_coupling_matrix(mask, lmax, bin_edges=None, use_namaster=True):
    ...
    use_namaster : bool
        If True, use NaMaster for MCM computation
    ...
    f = nmt.NmtField(mask, [mask], purify_b=False, lmax=lmax_effective)
...
def compute_power_spectra_master(maps_dict, mask, lmax=1024, bin_edges=None, 
    ...
    if verbose:
        print(f"Computing BNT power spectra for {len(bin_numbers)} bins with MASTER correction...")
...
def process_file(file_path, bnt_bin_range=[0, 1, 2, 3], noise_level=0.26, 
    ...
    """
    Process a single file: extract kappa maps, apply BNT transform, 
    apply mask, compute MASTER-corrected spectra.
    
    Key features:
    - Deterministic random seeds for reproducibility
    - Per-bin galaxy densities for shape noise
    - BNT transform applied before masking
    ...
    # Default galaxy densities (one per original bin)
    if galaxy_densities is None:
        galaxy_densities = [6.75] * 4  # Default for all 4 original bins
    
    # Construct output filename
    bnt_bin_str = "".join([str(b+1) for b in bnt_bin_range])  # Convert 0-based to 1-based
    ...
    spectra_type = "bnt_cross_cls" if cross_only else "bnt_all_cls"
    
    suffix = f"_{spectra_type}_bins{bnt_bin_str}{mask_suffix}{noise_suffix}{lmax_suffix}.npz"
    ...
    try:
        ...
        # Add shape noise BEFORE BNT transform (physically correct: noise is in original bins)
        if add_noise:
            for i in range(4):
                gal_density = galaxy_densities[i] if i < len(galaxy_densities) else 6.75
                kgs[i] = add_shape_noise(kgs[i], sigma_e=noise_level, 
        ...
        for bnt_bin in bnt_bin_range:
            maps_dict[bnt_bin + 1] = kgs_bnt[bnt_bin]  # Use 1-based indexing
        ...
        if verbose:
            ...
            print(f"Processed: {os.path.basename(file_path)} -> {len(cls_dict)} BNT {spectra_type_str} ({method})")
...
def aggregate_for_inference(processed_files, output_dir, bnt_bin_range=[0, 1, 2, 3],
    ...
    """
    Aggregate MASTER-corrected BNT .npz files into inference-ready .npy format.
    
    Validates that all files used consistent mask/correction settings.
    """
    print(f"\n{'='*60}")
    print(f"Aggregating {len(processed_files)} MASTER-corrected BNT files...")
    print(f"{'='*60}")
    
    # Initialize storage (use 1-based indexing for BNT bins)
    bnt_bins_1based = [b + 1 for b in bnt_bin_range]
    auto_spectra = {bin_num: [] for bin_num in bnt_bins_1based}
    cross_spectra = {(i, j): [] for i, j in combinations(sorted(bnt_bins_1based), 2)}
    ...
    for file_path in tqdm(processed_files, desc="Loading files"):
        try:
            ...
            for bin_num in bnt_bins_1based:
            ...
            for i, j in combinations(sorted(bnt_bins_1based), 2):
                key = f"cls_{i}_{j}"
                if key in data.files:
    ...
    # Save auto spectra
    print("\nSaving BNT auto power spectra...")
    for bin_num in bnt_bins_1based:
        if auto_spectra[bin_num]:
            auto_array = np.array(auto_spectra[bin_num])
            filename = f"all_bnt_cls_{dataset_type}_{map_type}_bin{bin_num}{mask_suffix}{noise_suffix}{lmax_suffix}.npy"
            output_path = os.path.join(output_dir, filename)
            np.save(output_path, auto_array)
            created_files.append(output_path)
            print(f"  ✓ BNT Bin {bin_num}: shape {auto_array.shape}")
    
    # Save combined cross spectra
    print("\nSaving BNT cross power spectra...")
    cross_data_parts = []
    ...
    for i, j in combinations(sorted(bnt_bins_1based), 2):
        if cross_spectra[(i, j)]:
            ...
            print(f"  ✓ BNT Cross ({i},{j}): shape {cross_array.shape}")
    
    if cross_data_parts:
        cross_combined = np.concatenate(cross_data_parts, axis=1)
        bnt_bin_str = "".join([str(b+1) for b in bnt_bin_range])
        filename = f"all_bnt_cross_cls_{dataset_type}_{map_type}_bins{bnt_bin_str}{mask_suffix}{noise_suffix}{lmax_suffix}.npy"
        output_path = os.path.join(output_dir, filename)
        np.save(output_path, cross_combined)
        created_files.append(output_path)
        print(f"\n  ✓ Combined BNT cross: shape {cross_combined.shape}")
...
def main():
    """Main function with enhanced CLI for BNT + MASTER mode-coupling correction."""
    parser = argparse.ArgumentParser(
        description="Process HEALPix maps with BNT transform and MASTER mode-coupling correction.",
    ...
    parser.add_argument("--bnt-bin-range", type=int, nargs="+", default=[0, 1, 2, 3],
                       help="BNT bins to process (0-indexed, default: 0 1 2 3).")
    ...
    print(f"\n{'='*60}")
    print(f"BNT + MASTER-Corrected Power Spectrum Processing")
    print(f"{'='*60}")
    print(f"Dataset: {dataset_type} {map_type}")
    print(f"Files: {len(file_paths)}")
    print(f"BNT Bins (0-indexed): {args.bnt_bin_range}")
