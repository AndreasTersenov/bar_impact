#!/usr/bin/env python3
"""
Cross Power Spectrum Aggregation Script - Aggregates cross power spectra .npz files into combined arrays.
"""
...
def find_cross_power_files(base_dir, pattern="*cross_cls_bins*.npz"):
    """
    Find all cross power spectrum files in a directory tree.
...
def load_and_validate_file(file_path, expected_keys=None, verbose=False):
    """
    Load and validate a cross power spectrum file.
    
    Parameters:
    - file_path: str, path to the .npz file
    - expected_keys: list, expected cross power spectrum keys
    ...
    try:
        ...
        if not cls_data:
            if verbose:
                print(f"No cross power spectra found in {os.path.basename(file_path)}")
            return None
...
def aggregate_cross_power_spectra(file_paths, bin_range=[1, 2, 3, 4], cross_only=True, verbose=False):
    """
    Aggregate cross power spectra from multiple files.
    
    Parameters:
    - file_paths: list, paths to .npz files
    - bin_range: list, expected bin numbers
    ...
    if cross_only:
        expected_keys = [f"cls_{i}_{j}" for i, j in combinations(bin_range, 2)]
    else:
        # Include auto spectra
        auto_keys = [f"cls_{i}_{i}" for i in bin_range]
        cross_keys = [f"cls_{i}_{j}" for i, j in combinations(bin_range, 2)]
    ...
    for file_path in tqdm(file_paths, desc="Loading files"):
        data = load_and_validate_file(file_path, expected_keys, verbose)
        if data is not None:
    ...
    print(f"Loaded {len(valid_data)} valid files, skipped {skipped_files} files")
    ...
    # Initialize arrays for each cross power spectrum
    ...
    if not cross_only:
        ...
        auto_keys = [f"cls_{i}_{i}" for i in sorted(bin_range)]
    ...
    # Add cross spectra
    cross_keys = [f"cls_{i}_{j}" for i, j in combinations(sorted(bin_range), 2)]
...
def main():
    """Main function to handle command-line arguments and run aggregation."""
    parser = argparse.ArgumentParser(description="Aggregate cross power spectra .npz files into combined arrays.")
    
    # Input options
    parser.add_argument("--base-dir", required=True,
                        help="Base directory to search for cross power spectrum files.")
    parser.add_argument("--pattern", default="*cross_cls_bins*.npz",
                        help="Glob pattern to match files (default: *cross_cls_bins*.npz).")
    parser.add_argument("--bin-range", type=int, nargs="+", default=[1, 2, 3, 4],
                        help="Expected bin range (default: 1 2 3 4).")
    ...
    # Find files
    print(f"Searching for files in {args.base_dir} with pattern {args.pattern}")
    file_paths = find_cross_power_files(args.base_dir, args.pattern)
    ...
    result = aggregate_cross_power_spectra(
        file_paths, 
    ...
    print(f"  Bin range: {metadata['bin_range']}")
    print(f"  Cross spectra only: {metadata['cross_only']}")
    print(f"  Available spectra:")
    ...
    if not args.output:
        ...
        if args.inference_compatible and args.output_format == "npy":
            # Generate filename compatible with run_npe_inference_ps.py
            if metadata['cross_only']:
                # For cross-only, use a special identifier
                filename = f"all_cross_cls_{dataset_info}_{map_info}_bins{bin_str}{noise_suffix}"
            else:
                # For auto+cross, use standard naming
                filename = f"all_cls_{dataset_info}_{map_info}_bins{bin_str}{noise_suffix}"
        else:
            # Use descriptive naming for other formats
            spectra_type = "cross" if metadata['cross_only'] else "all"
            filename = f"aggregated_{spectra_type}_cls_{dataset_info}_{map_info}_bins{bin_str}{noise_suffix}"
    ...
    if args.output_format == "npy":
        ...
        if not args.inference_compatible:
            ...
            print(f"Saved detailed data to: {metadata_file}")
        ...
        with h5py.File(args.output, 'w') as f:
            # Create groups
            spectra_group = f.create_group('cross_power_spectra')
