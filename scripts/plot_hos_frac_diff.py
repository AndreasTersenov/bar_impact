...
def main():
    ...
    plt.rcParams.update({"legend.fontsize": 11, "axes.labelsize": 13,
                         "xtick.labelsize": 11, "ytick.labelsize": 11})
    fig, axes = plt.subplots(1, len(scales), figsize=(4.3 * len(scales), 3.3), sharey=True)
    ...
    for ax, s, per_bin in zip(axes, scales, panel):
        ...
        ax.set_xlabel("SNR")
        ax.set_title(f"Wavelet Scale {s+1}", fontsize=12)
        for sp in ("top", "right"):
    ...
    fig.legend(h, l, loc="lower center", bbox_to_anchor=(0.5, 0.93), ncol=4, frameon=False)
    fig.tight_layout()
