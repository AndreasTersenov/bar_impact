#!/usr/bin/env python3
"""
Submit NPE inference jobs for L1-norm parameter sweep - PARALLEL VERSION.
...
- scales (wavelets): "1,2,3,4" and "2,3,4"

Jobs are distributed across GPUs 0, 1, 2 and run in parallel (one job per GPU at a time).

Usage:
  python scripts/submit_npe_inference_l1_parameter_sweep_parallel.py [--run N] [--rerun] [--bnt]
...
# Fixed parameters
BASE_CMD = "python scripts/run_npe_inference.py"
FIXED_PARAMS = {
...
# Log directory
LOG_DIR = Path("logs/npe_l1_parameter_sweep")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Submit NPE L1-norm inference parameter sweep jobs")
    ...
    if RUN_NUMBER is not None:
        LOG_DIR = Path(f"logs/npe_l1_parameter_sweep{bnt_suffix}_run{RUN_NUMBER}")
    elif USE_BNT:
        LOG_DIR = Path(f"logs/npe_l1_parameter_sweep{bnt_suffix}")
    ...
    print("="*80)
    print("NPE L1-Norm Inference Parameter Sweep - PARALLEL EXECUTION")
    if USE_BNT:
