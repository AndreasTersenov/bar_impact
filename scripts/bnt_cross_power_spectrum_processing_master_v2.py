#!/usr/bin/env python3
"""
BNT Cross Power Spectrum Processing Script with MASTER Correction (Refactored Version)

This script applies Band-limited Nulling Transform (BNT) to cosmological maps
and computes cross power spectra with proper MASTER mode-coupling correction.

Key features:
- Uses bar_impact.processing.master_correction for MASTER algorithm
- Leverages bar_impact.processing.bnt_transforms for BNT
- Uses bar_impact.core.masks for C1/C2 apodization
...
- Uses centralized BNT transform utilities
- Leverages MASTER correction from package
...
def process_file(
    ...
    bnt_bin_range : list, optional
        BNT bins to process (0-indexed, 0-3)
    noise_level : float, optional
        Shape noise level (sigma_e)
    galaxy_densities : list, optional
        Per-bin galaxy densities in arcmin^-2 (for original bins 1-4)
    add_noise : bool, optional
        Whether to add shape noise (before BNT transform)
    lmax : int, optional
        Maximum multipole
    ...
    # Default galaxy densities (for original 4 bins)
    if galaxy_densities is None:
        galaxy_densities = [DEFAULT_GALAXY_DENSITY] * 4
    
    # Construct output filename
    bnt_bin_str = "".join([str(b+1) for b in bnt_bin_range])  # 1-based for display
    ...
    try:
        ...
        # Add shape noise BEFORE BNT transform
        # (physically correct: noise is in original bins)
        if add_noise:
            for i in range(4):
                kg_maps[i] = add_shape_noise(
                    kg_maps[i],
                    sigma_e=noise_level,
                    galaxy_density=galaxy_densities[i],
                    nside=DEFAULT_NSIDE,
                    rng=rng,
                )
        ...
        # Create maps dictionary for specified BNT bins
        maps_dict = {}
        for bnt_bin in bnt_bin_range:
            # Use 1-based indexing for display (BNT bins 1-4)
            maps_dict[bnt_bin + 1] = bnt_maps[bnt_bin]
        ...
        save_dict['bin_range'] = np.array(list(maps_dict.keys()))  # 1-based BNT bins
        save_dict['lmax'] = lmax
        save_dict['ells'] = ells
        ...
        if verbose:
            ...
            print(f"Processed {os.path.basename(file_path)}: {n_spectra} BNT spectra")
...
def main():
    """Main function with CLI for BNT cross power spectrum processing."""
    parser = argparse.ArgumentParser(
        description="Process HEALPix maps with BNT transform to compute cross power spectra with MASTER correction.",
    ...
    parser.add_argument("--bnt-bin-range", type=int, nargs="+", default=[0, 1, 2, 3],
                       help="BNT bins to process (0-indexed, 0-3).")
    ...
    parser.add_argument("--galaxy-densities", type=float, nargs="+",
                       help=f"Per-bin galaxy densities in arcmin^-2 (default: {DEFAULT_GALAXY_DENSITY} for all 4 bins).")
    ...
    for bnt_bin in args.bnt_bin_range:
        if bnt_bin not in [0, 1, 2, 3]:
            print(f"Error: BNT bin {bnt_bin} is invalid. Must be 0-3.")
            sys.exit(1)
    ...
    print(f"\n{'='*60}")
    print(f"BNT Cross Power Spectrum Processing (Refactored Version)")
    print(f"{'='*60}")
