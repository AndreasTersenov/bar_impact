...
This script runs ONLY the new upper-cut range: 340 to 500 (step 20).
Use this to extend existing runs without re-running upper-cut >= 520.

Jobs are distributed across GPUs 0, 1, 2 and run in parallel (one job per GPU at a time).

Usage:
  python scripts/submit_npe_inference_parameter_sweep_parallel_340_500.py [--run N]
...
UPPER_CUTS = list(range(340, 501, 20))  # 340, 360, 380, 400, 420, 440, 460, 480, 500
MASK_AREAS = [2000.0, 5000.0, 10000.0, 14000.0, 28000.0, 35000.0]
GPUS = [0, 1, 2]

# Log directory
LOG_DIR = Path("logs/npe_parameter_sweep_340_500")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Submit NPE inference parameter sweep jobs (340-500 only)")
    ...
    if RUN_NUMBER is not None:
        LOG_DIR = Path(f"logs/npe_parameter_sweep_340_500_run{RUN_NUMBER}")
    ...
    # Filter out jobs that already have output files
    jobs_to_run = []
    skipped_jobs = []
    ...
    print("*** PARTIAL RUN: upper-cut 340-500 only ***")
    if RUN_NUMBER is not None:
