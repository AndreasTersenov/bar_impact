#!/usr/bin/env python3
# filepath: /lustre/fsn1/projects/rech/prk/ulx34io/bar_impact/scripts/run_npe_inference_ps.py
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Run NPE inference on CosmoGRID power spectra")
...
def main():
    ...
    for cls_full in cls_full_bins:
        ...
        processed_data_list.append(cls_processed)
    
    # Concatenate all bins together
    data_vector = np.concatenate(processed_data_list, axis=1)
    
    print(f"Combined datavector shape: {data_vector.shape}")

    # Convert to JAX arrays
    params = jnp.array(params)
    data_vector = jnp.array(data_vector)
    ...
    # Initialize NPE
    inference = NPE()
    inference = inference.append_simulations(params, data_vector)
    ...
    for fid_mean in fid_means:
        ...
        fid_data_list.append(fid_processed)
    
    # Concatenate all bins' fiducial data
    fid_mean_processed = np.concatenate(fid_data_list)
    print(f"Combined fiducial data shape: {fid_mean_processed.shape}")
    ...
    samples = posterior.sample(
        x=fid_mean_processed, num_samples=num_samples, key=sample_key
    )
    ...
    sample_label = f"{args.simulation_type} DV vs {fiducial_desc} fid, {bin_spec}, {ps_desc}"
