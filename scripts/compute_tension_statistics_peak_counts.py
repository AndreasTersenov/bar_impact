#!/usr/bin/env python3
"""
Compute tension statistics between nobaryons and baryonified fiducial posteriors for peak counts.
...
Usage:
  python scripts/compute_tension_statistics_peak_counts.py [--run N] [--bnt]
...
# Parameter ranges (matching submit_npe_peak_counts_parameter_sweep_parallel.py)
...
def main():
    ...
    print("="*80)
    print("Computing Peak Counts Tension Statistics: nobaryons vs baryonified")
    if USE_BNT:
    ...
    csv_full = output_dir / f"tension_pc{bnt_suffix}_nobaryons_vs_baryonified_full_6params{run_suffix}.csv"
    csv_subset = output_dir / f"tension_pc{bnt_suffix}_nobaryons_vs_baryonified_subset_3params{run_suffix}.csv"
    ...
    if len(df_full) > 0:
        ...
        # Save pivot table
        pivot_csv_full = output_dir / f"tension_pc{bnt_suffix}_nobaryons_vs_baryonified_full_6params_pivot{run_suffix}.csv"
    ...
    if len(df_subset) > 0:
        ...
        # Save pivot table
        pivot_csv_subset = output_dir / f"tension_pc{bnt_suffix}_nobaryons_vs_baryonified_subset_3params_pivot{run_suffix}.csv"
