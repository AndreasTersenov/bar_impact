...
def test_aggregation():
    ...
    try:
        ...
        test_files = create_test_npz_files(
            test_dir, n_files=n_files, bin_range=bin_range, lmax=lmax
        )
        ...
        for bin_num in bin_range:
            expected_file = os.path.join(
                output_dir, f"all_cls_grid_nobaryons_bin{bin_num}.npy"
            )
            if os.path.exists(expected_file):
                ...
                if data.shape == expected_shape:
                    print(
                        f"  ✓ Bin {bin_num}: {os.path.basename(expected_file)} - shape {data.shape} ✓"
                    )
                else:
                    print(
                        f"  ✗ Bin {bin_num}: Wrong shape {data.shape}, expected {expected_shape}"
                    )
                    success = False
            else:
        ...
        expected_cross_file = os.path.join(
            output_dir, f"all_cross_cls_grid_nobaryons_bins{bin_str}.npy"
        )
        if os.path.exists(expected_cross_file):
            ...
            if data.shape == expected_shape:
                print(
                    f"  ✓ Cross: {os.path.basename(expected_cross_file)} - shape {data.shape} ✓"
                )
            else:
